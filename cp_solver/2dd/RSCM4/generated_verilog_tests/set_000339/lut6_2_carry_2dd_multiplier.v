...
module lut6_2_carry_2dd_multiplier (
    ...
    localparam integer ADD1_Y_BW = 5;
    ...
    // requested coeffs sorted = [-9, -8, -7, -6, -3, -2, -1, 0, 3, 4, 5, 6, 7, 8, 9, 10]
    // selector coeffs        = [5, 9, -7, -1, 6, 10, -6, 0, 3, 7, -9, -3, 4, 8, -8, -2]
    // add1 coeffs            = [1, 2, -1, 0]
    ...
    wire signed [4:0] add1_y;

    // add1: rows reordered for op select bit: false
    // add1: pack mode: lut-di
    // add1: correction carry-in is add1_sel[1]
    wire [7:0] add1_s;
    wire [7:0] add1_di;
    wire [7:0] add1_o;
    wire [7:0] add1_co;
    wire add1_carry_init = add1_sel[1];
    ...
    localparam [63:0] ADD1_BIT_0_S_INIT =
        (ADD1_SEL_ROW_0 & (LUT_I2 ^ 64'h0000000000000000)) |
        (ADD1_SEL_ROW_1 & (LUT_I3 ^ 64'h0000000000000000)) |
        (ADD1_SEL_ROW_2 & (~LUT_I3 ^ LUT_I2)) |
        (ADD1_SEL_ROW_3 & (~LUT_I4 ^ LUT_I4));
    localparam [63:0] ADD1_BIT_0_DI_INIT =
        (ADD1_SEL_ROW_0 & (LUT_I2)) |
    ...
    assign add1_s[5] = 1'b0;
    assign add1_di[5] = 1'b0;
