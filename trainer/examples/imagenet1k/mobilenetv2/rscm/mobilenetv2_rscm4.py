...
from hatorch.models import MobileNet
...
OUTPUT_DIR = "results/MOBILENETV2_RSCM4/"
FINAL_MODEL_PATH = str(Path(OUTPUT_DIR) / "mobilenetv2_rscm4_final.pt")
DEVICE = "cuda"
BATCH_SIZE = 256
SEED = 42
...
RSCM_SET_PATH = str(Path(__file__).resolve().parents[3] / "sets" / "RSCM4.txt")
...
def create_quantization_parameters():
    return QuantizationRecipe(
        ...
        activations=ActivationQuantization(
            ...
            affine_zero_point=False,
            rounding=ACTIVATION_ROUNDING_MODE,
        ),
        bias=BiasQuantization(bits=BIAS_BITS),
