...
from hatorch.models import ResNet18
...
OUTPUT_DIR = "results/RESNET18_RSCM4/"
FINAL_MODEL_PATH = str(Path(OUTPUT_DIR) / "resnet18_rscm4_final.pt")
DEVICE = "cuda"
BATCH_SIZE = 512
...
MASTER_PORT = "12355"
RSCM_SET_PATH = str(Path(__file__).resolve().parents[2] / "sets" / "RSCM4.txt")
...
SCALE_APPROXIMATION = ScaleApproximation.POWER_OF_TWO
SCALE_APPROXIMATION_START_EPOCH = 12
...
def create_quantization_parameters():
    return QuantizationRecipe(
        ...
        transforms=TransformPolicy(
            ...
            round_average_pool_output=True,
        ),
    ).build()
