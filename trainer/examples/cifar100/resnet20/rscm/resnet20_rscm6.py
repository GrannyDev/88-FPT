"""ResNet20/CIFAR100 RSCM6 QAT recipe with sigmoid staircase weight quantization.

This example keeps the training schedule identical to the matching INT recipe,
but replaces uniform middle-layer weights with autoset RSCM codebooks using
the non-uniform sigmoid staircase quantizer.
"""
...
from hatorch.models import ResNet20
...
OUTPUT_DIR = "results/RESNET20_CIFAR100_RSCM6/"
FINAL_MODEL_PATH = str(Path(OUTPUT_DIR) / "resnet20_rscm6_final.pt")
DEVICE = "cuda"
BATCH_SIZE = 256
...
WEIGHT_QUANTIZER_KIND = WeightQuantizerKind.NON_UNIFORM_SIGMOID_STAIRCASE
AUTOSET_OBJECTIVE = AutosetObjective.OUTPUT_MSE

SIGMOID_STAIRCASE_CONFIG: SigmoidStaircaseConfig | None = SigmoidStaircaseConfig(
    start_value=2.0,
    end_value=32.0,
    start_epoch=3,
    end_epoch=25,
...
ACTIVATION_BITS = 6
WEIGHTS_BITS = 6
RSCM_SET_PATH = str(Path(__file__).resolve().parents[4] / "sets" / f"RSCM{WEIGHTS_BITS}.txt")
...
WEIGHT_QUANTIZATION = WeightQuantization.autoset_codebook(
    path=RSCM_SET_PATH,
    coefficients=1 << WEIGHTS_BITS,
    kind=WEIGHT_QUANTIZER_KIND,
    per_channel=PER_CHANNEL_WEIGHTS,
    batches=1,
    objective=AUTOSET_OBJECTIVE,
    sigmoid_staircase=SIGMOID_STAIRCASE_CONFIG,
)
